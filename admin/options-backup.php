...
// End init()
    ...
    function register_admin_screen() {
        ...
        $this->admin_page = add_theme_page( __( 'Cumico Setting Import / Export', 'okthemes' ), __( 'Import / Export', 'okthemes' ), 'manage_options', $this->token, array( &$this, 'admin_screen' ) );
    ...
    function admin_screen() {
        ...
        <div class="wrap">
            ...
            <h2><?php _e( 'Import / Export', 'okthemes' ); ?></h2>
            <div class="import">
                <h3><?php _e( 'Import Settings', 'okthemes' ); ?></h3>
                <p><?php _e( 'If you have settings in a backup file on your computer, the Import / Export system can import those into this site. To get started, upload your backup file to import form below.', 'okthemes' ); ?></p>
                ...
                <form enctype="multipart/form-data" method="post" action="<?php echo admin_url( 'admin.php?page=' . $this->token ); ?>">
                    ...
                    <label for="OptionsFramework-import-file"><?php printf( __( 'Upload File: (Maximum Size: %s)', 'okthemes' ), ini_get( 'post_max_size' ) ); ?></label>
                    ...
                    <input type="submit" class="button" value="<?php _e( 'Upload File and Import', 'okthemes' ); ?>" />
                </form>

            </div>
            <div class="export">
                <h3><?php _e( 'Export Settings', 'okthemes' ); ?></h3>
                <p><?php _e( 'When you click the button below, the Import / Export system will create a text file for you to save to your computer.', 'okthemes' ); ?></p>
                <p><?php echo sprintf( __( 'This text file can be used to restore your settings here on "%s", or to easily setup another website with the same theme settings.', 'okthemes' ), get_bloginfo( 'name' ) ); ?></p>
                <form method="post" action="<?php echo admin_url( 'admin.php?page=' . $this->token ); ?>">
                    ...
                    <input type="submit" class="button" value="<?php _e( 'Download Export File', 'okthemes' ); ?>" />
                </form>
            </div>
...
// End admin_screen()
    ...
    function admin_screen_help() {
        ...
        $screen->add_help_tab( array(
            ...
            'title' => __( 'Backup Manager', 'okthemes' ),
            'content' => '<h3>' . __( 'Welcome to the OptionsFramework Backup Manager.', 'okthemes' ) . '</h3>' .
            '<p>' . __( 'Here are a few notes on using this screen.', 'okthemes' ) . '</p>' .
            '<p>' . __( 'The backup manager allows you to backup or restore your "Theme Options" and other settings to or from a text file.', 'okthemes' ) . '</p>' .
            '<p>' . __( 'To create a backup, simply hit the "Download Export File" button.', 'okthemes' ) . '</p>' .
            '<p>' . __( 'To restore your settings from a backup, browse your computer for the file (under the "Import Settings" heading) and hit the "Upload File and Import" button. This will restore only the settings that have changed since the backup.', 'okthemes' ) . '</p>' .
            '<p><strong>' . __( 'Please note that only valid backup files generated through the OptionsFramework Backup Manager should be imported.', 'okthemes' ) . '</strong></p>'
        ) );
    }
...
// End admin_screen_help()
    ...
    function admin_notices() {
        ...
        echo '<div id="import-notice" class="updated"><p>' . sprintf( __( 'Please note that this backup manager backs up only your theme settings and not your content. To backup your content, please use the %sWordPress Export Tool%s.', 'okthemes' ), '<a href="' . admin_url( 'export.php' ) . '">', '</a>' ) . '</p></div><!--/#import-notice .message-->' . "\n";
        ...
        if ( isset( $_GET['error'] ) && $_GET['error'] == 'true' ) {
            ...
            echo '<div id="message" class="error"><p>' . __( 'There was a problem exporting your settings. Please Try again.', 'okthemes' ) . '</p></div>';
        } else if ( isset( $_GET['invalid'] ) && $_GET['invalid'] == 'true' ) {
            echo '<div id="message" class="error"><p>' . __( 'The import file you\'ve provided is invalid. Please try again.', 'okthemes' ) . '</p></div>';
        } else if ( isset( $_GET['imported'] ) && $_GET['imported'] == 'true' ) {
            echo '<div id="message" class="updated"><p>' . sprintf( __( 'Settings successfully imported. | Return to %sTheme Options%s', 'okthemes' ), '<a href="' . admin_url( 'admin.php?page=options-framework' ) . '">', '</a>' ) . '</p></div>';
        } // End IF Statement
    }
